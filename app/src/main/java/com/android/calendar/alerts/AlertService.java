...
public class AlertService extends Service {
    ...
    protected boolean hasRequiredPermission(String[] permissions) {//PERMISSION_GRANTED授予 PERMISSION_DENIED否认
    ...
    void processMessage(Message msg) {
        ...
        boolean providerReminder = action.equals(
                android.provider.CalendarContract.ACTION_EVENT_REMINDER);//事件提醒
        if (providerReminder) {
            ...
            //TRANSSION BEGIN  play defalut ringtone when in call state liaohuan-2016.11.16
           if (mTelephonyManager != null && mTelephonyManager.getCallState() != TelephonyManager.CALL_STATE_IDLE) {
              final Ringtone ringtone = RingtoneManager.getRingtone(this, Uri.parse(GeneralPreferences.DEFAULT_RINGTONE));  
               if (ringtone != null) {
    ...
    public static boolean generateAlerts(Context context, NotificationMgr nm,
        ...
        if (numLowPriority > 0) {
            ...
            addNotificationOptions(notification, true, expiredDigestTitle,
                    true,notificationPrefs.getRingtoneAndSilence(),
...
//                    cr.update(alertUri, values, null, null);
    ...
    //添加通知选项，通知类；安静与否；钟表，断续器；震动；提醒铃声；闪光灯
    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            boolean hasSelfPermission = Settings.System.canWrite(AlertService.this);
