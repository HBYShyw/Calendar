...
public class Utils {
    ...
    /* HYW: bug limit date between 1970/01/01 and 2037/12/31 @{ */
    ...
    **/
    ...
    // each day.
    /*一个单链表示一种颜色的事件。事件被除以颜色使他们方便地绘制。
    黑链是特别，原因是它拥有冲突事件，以及 allday 颜色设置在每一天。*/
    ...
    public static void setTodayIcon(LayerDrawable icon, Context c, String timezone) {
        ...
        if (currentDrawable != null && currentDrawable instanceof DayOfMonthDrawable) {
            ...
            //today.setBounds(0, 0, 100, 100);//第一0是距左边距离，第二0是距上边距离，40分别是长宽
        } else {
        ...
        now.setToNow();
        now.normalize(false);
        today.setDayOfMonth(now.monthDay);//设置当天多少号
        icon.mutate();
    ...
    public static long setJulianDayInGeneral(Time time, int julianDay) {
        int dif = julianDay - Time.EPOCH_JULIAN_DAY;//week*7-3
