...
public class MonthByWeekAdapter extends SimpleWeeksAdapter {
    ...
    protected int mClickSelectedWeek;
    /// @}
    ...
    int mClickedDayIndex = -1;
    float mClickedXLocation;                // Used to find which day was clicked 用来寻找哪一天被点击 坐标
    ...
    protected void init() {
        ...
        mClickSelectedWeek = mSelectedWeek;
        /// @}
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (mRealSelectedWeek == position) {
            ...
            Log.e("666", "getView() mRealSelectedDay.monthDay="+mRealSelectedDay.monthDay+",mRealSelectedDay.month="+mRealSelectedDay.month
                    +",mTempTime.month="+mTempTime.month+",mTempTime.monthDay"+mTempTime.monthDay+
                    "mRealSelectedDay.weekDay="+mRealSelectedDay.weekDay+"mRealSelectedWeek= "+mRealSelectedWeek);
        }
        /// @}

        //点击的日期在这个星期
        int clickSelectedIndex= -1;
        if(mClickSelectedWeek == position){
            clickSelectedIndex = mClickedDayIndex;
        }
        ...
        if (mSelectedWeek == position) {
            ...
            Log.e("666", "getView() mSelectedDay.monthDay="+mSelectedDay.monthDay+",mSelectedDay.month="+mSelectedDay.month
                    +",mTempTime.month="+mTempTime.month+",mTempTime.monthDay"+mTempTime.monthDay+
                    "mSelectedDay.weekDay="+mSelectedDay.weekDay+"mRealSelectedWeek= "+mRealSelectedWeek);
        }
        ...
        drawingParams.put(SimpleWeekView.VIEW_PARAMS_SELECTED_DAY, realSelectedWeekDay);//selectedDay 传入选择的天
        drawingParams.put(SimpleWeekView.VIEW_PARAMS_CLICK_SELECTED_INDEX, clickSelectedIndex);//点击的天位置
        drawingParams.put(SimpleWeekView.VIEW_PARAMS_SHOW_WK_NUM, mShowWeekNumber ? 1 : 0);
        drawingParams.put(SimpleWeekView.VIEW_PARAMS_WEEK_START, mFirstDayOfWeek);//一周开始
        drawingParams.put(SimpleWeekView.VIEW_PARAMS_NUM_DAYS, mDaysPerWeek);
        drawingParams.put(SimpleWeekView.VIEW_PARAMS_WEEK, position);
        drawingParams.put(SimpleWeekView.VIEW_PARAMS_FOCUS_MONTH, mFocusMonth);
        drawingParams.put(MonthWeekEventsView.VIEW_PARAMS_ORIENTATION, mOrientation);//方向参数，默认竖屏
    ...
    protected void refresh() {
        mFirstDayOfWeek = Utils.getFirstDayOfWeek(mContext);//获得一个星期的起始天
    ...
    protected void onDayTapped(Time day) {
        Log.d("666", TAG+"111[  onDayTapped---start1" +",day:" + day);
        //20160101T000000Africa/Lagos(5,0,3600,0,1451602800)
        setDayParameters(day);
        Log.d("666", TAG+"111[  onDayTapped---start2" +",day:" + day);
        //20160101T070000Africa/Lagos(5,0,3600,0,1451628000)
         ...
         if (mShowAgendaWithMonth || mIsMiniMonth) {
            ...
            // with the selected day's info
             Log.d("666", TAG+"111[  onDayTapped---start" +",day1:" + day);
            ...
            if (!mIsMiniMonth) {//false
                //跳转到日视图
                Log.d("666", TAG+"111[  onDayTapped---start" +",day2:" + day);
                ...
                Log.d("666", TAG+"111[  onDayTapped---start" +",day3:" + day);
                mController.sendEvent(mContext, EventType.GO_TO, day, day, -1, mIsMiniMonth ? ViewType.CURRENT
        ...
        if (mGestureDetector.onTouchEvent(event)) {
            ...
            switch (action) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    //hyw
                    mClickedDayIndex = (int) event.getX();
                    //hyw
    ...
    // Clear the visual cues of the click animation and related running code. 清理点击天的坐标
    ...
    private final Runnable mDoSingleTapUp = new Runnable() {
        ...
        public void run() {
            if (mSingleTapUpView != null) {
                Time day = mSingleTapUpView.getDayFromLocation(mClickedXLocation);//根据当前点击位置确定点击的是哪天的坐标，然后确定是日期哪一天
                if (Log.isLoggable(TAG, Log.DEBUG)) {
                    Log.d(TAG, "Touched day at Row=" + mSingleTapUpView.mWeek + " day=" + day.toString());
                    Log.d("666", TAG+"111[  onTouch---start1" +",mSingleTapUpView.mWeek:" + mSingleTapUpView.mWeek
                            + " day=" + day.toString());
                }
                if (day != null) {
                    Log.d("666", TAG+"111[  Runnable---start1" +",day:" + day+"mSingleTapUpView.mWeek:"+mSingleTapUpView.mWeek);
                    onDayTapped(day);
                    Log.d("666", TAG+"111[  Runnable---start2" +",day:" + day+"mSingleTapUpView.mWeek:"+mSingleTapUpView.mWeek);
                }
