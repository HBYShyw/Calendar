...
public class MonthListView extends ListView {
    ...
    private boolean processEvent (MotionEvent ev) {
        switch (ev.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_CANCEL:
                return false;
            // Start tracking movement velocity /*跟踪运动速度*/
    ...
    private void doFling(float velocityY) {
        ...
        mTempTime.setJulianDay(day); /*设置前后月的日期*/
        //设置时间为给定的儒历日，前提是必须处于同一时区；
        mTempTime.monthDay = 1;
        mTempTime.month += monthsToJump;
        long timeInMillis = mTempTime.normalize(false); // 规范化
        ...
        /*因为每个视图是7天,在目标的一天,以确保滚动将至少一个视图*/
        // HYW: Add for bug,Month view slide error.
