...
public class SimpleWeekView extends View {
     ...
     */
    //HYW
    public static final String VIEW_PARAMS_CLICK_SELECTED_INDEX = "click_index";
    //HYW
     ...
     * [0-11].
     */
    public static final String VIEW_PARAMS_FOCUS_MONTH = "focus_month";//获得焦点的月份
    /**
    ...
    protected static final int DEFAULT_WEEK_START = Time.SUNDAY;//默认一周开始（周日）
    ...
    // Which day is selected [0-6] or -1 if no day is selected ,选择的天，默认-1
    ...
    // is)
    protected String mTimeZone = Time.getCurrentTimezone();//时区

    //TRANSSION BEGIN
    protected boolean mShowSelectDayColor = false;
    protected int mClickSelectIndex = -1;
    //TRANSSION END
    ...
    public SimpleWeekView(Context context) {
        ...
        //TRANSSION BEGIN
        mShowSelectDayColor = res.getBoolean(R.bool.show_selectday_color);//true
        //TRANSSION END
     ...
     * @param tz The time zone this view should reference times in 参考
     */
    public void setWeekParams(HashMap<String, Integer> params, String tz) {
        ...
        if (params.containsKey(VIEW_PARAMS_SELECTED_DAY)) {
            ...
            Log.d("2323", "setWeekParams_comming*****mSelectedDay:" + mSelectedDay);
        }

        //hyw
        if(params.containsKey(VIEW_PARAMS_CLICK_SELECTED_INDEX)){
            mClickSelectIndex = params.get(VIEW_PARAMS_CLICK_SELECTED_INDEX);
            Log.d("2323", "setWeekParams_comming*****mClickSelectIndex:" + mClickSelectIndex);
        }
        //hyw
        ...
        if (params.containsKey(VIEW_PARAMS_SHOW_WK_NUM)) { //show_wk_num
        ...
        mNumCells = mShowWeekNum ? mNumDays + 1 : mNumDays;// 7天
        ...
        mWeek = params.get(VIEW_PARAMS_WEEK);//选中的item的position，week
        ...
        // If the week is set to start on a Saturday the first week will be 星期六
        ...
        if (time.weekDay != mWeekStart) {
            ...
            time.monthDay -= diff;
            Log.d("2222", "setWeekParams monthDay****"+ time.monthDay);
            time.normalize(true);
        }

        mFirstJulianDay = Time.getJulianDay(time.toMillis(true), time.gmtoff);
        mFirstMonth = time.month;
        Log.d("2222", "setWeekParams month****"+ time.month);
        ...
        for (; i < mNumCells; i++) {
            ...
            if (time.year == today.year && time.yearDay == today.yearDay) {//确认是今天为默认选择的天
                mHasToday = true;
                mToday = i;
            }
            //AA--得到天数的数组drawWeekNums()    [1,31]
            mDayNumbers[i] = Integer.toString(time.monthDay++);// int day = time.monthDay;
    ...
    public Time getDayFromLocation(float x) {
        ...
        Log.d("333", "111[ simpleWeekView getDayFromLocation---start" + dayPosition+"day:"+day+"time"+time.year+","+time.month+","+time.monthDay);
