...
/*这个类需要在界面设定之前实例化，因为界面的显示离不开数据，
对于日历来说，界面的显示，离不开CalendarController类的数据。
CalendarController主要是控制了event事件的类型，开始时间，结
束时间，等。它贯穿了日历的整个数据处理过程。

说白了在我的理解中，CalendarController它就是被白话了的数据库。
不管运行到日历的哪个activity，它都能随意存取，是数据处理的全局变量作用的一个基类。
*/
...
/*
CalendarController从字面上来理解应该叫做控制器，我们知道日历的很多事件响应都是在AllInOneActivity中发生的，
但是我们的处理却不是在这里，而是在响应的视图对应的activity或者fragment之中，如何让捕获到事件的AllInOneActivity
去通知相应的fragment去处理这些事件呢，就是通过CalendarController 。CalendarController相当于一个中转站。
CalendarController中定义了一个叫EventHandler的接口：
AllInOneActivity本身继承了EventHandler并实现了handleEvent方法，
同时各个fragment也继承了继承了EventHandler并实现了handleEvent方法，
这样就能在CalendarController随意的调用各个模块。
*/
...
public class CalendarController {
    ...
    public void sendEvent(Object sender, final EventInfo event) {
        ...
        Log.d("666", TAG+"111[  sendEvent---start" +",event.startTime: " + event.startTime);
        Log.d("666", TAG+"111[  sendEvent---start" +",event.endTime: " + event.endTime);
        Log.d("666", TAG+"111[  sendEvent---start" +",event.selectedTime: " + event.selectedTime);
        Log.d("666", TAG+"111[  sendEvent---start" +",mTime: " + mTime);
        ///@}
