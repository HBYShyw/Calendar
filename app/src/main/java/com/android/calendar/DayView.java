...
public class DayView extends View implements View.OnCreateContextMenuListener,
    ...
    //AA--日历中所有时间的设置都使用Time并开启一个子线程进行更新
...
//        mNewEventHintString = mResources.getString(R.string.day_view_new_event_hint);
        mNewEventHintString = mResources.getString(R.string.event_create);
        mNumDays = numDays;
    ...
    private void init(Context context) {
        ...
        mFutureBgColorRes = mResources.getColor(R.color.calendar_future_bg_color); //当前时间之后的区域色值
        ...
        mCalendarGridAreaSelected = mResources.getColor(R.color.calendar_grid_area_selected); //点击区域颜色
        mCalendarGridLineInnerHorizontalColor = mResources
                .getColor(R.color.calendar_grid_line_inner_horizontal_color);  //header下划线的颜色
        ...
        Log.d(TAG, "init()[mLines] " + maxGridLines);
...
//                Trace.traceCounter(Trace.TRACE_TAG_PERF, CALENDAR_APPUPDATE, 0);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.translate(-mViewStartX, yTranslate);//AA--可以实现view的平移。
        ...
        // Draw the movable part of the view 绘制移动部分视图
        doDraw(canvas);
    ...
    private void drawDayHeaderLoop(Rect r, Canvas canvas, Paint p) {
        ...
        // canvas.drawRect(r, p);
        if (mNumDays == 1 && ONE_DAY_HEADER_HEIGHT == 0) {  //绘制上面周期
            return;
        }
    ...
    private void drawCurrentTimeLine(Rect r, final int day, final int top, Canvas canvas,
            Paint p) {
        //huangyongwen@20161230 add for calendar new UI begin
        ...
        r.right = computeDayLeftPosition(day + 1)+1 ;

        //        r.top = top - CURRENT_TIME_LINE_TOP_OFFSET;
        //        r.bottom = r.top + mCurrentTimeLine.getIntrinsicHeight();
        r.top = top - CURRENT_TIME_LINE_TOP_OFFSET+1;
        r.bottom = top + CURRENT_TIME_LINE_TOP_OFFSET-1;
        Log.d("syg", "r.top="+r.top+",r.bottom="+r.bottom+",top="+top);
        int underlineColor = mResources.getColor(R.color.currenttimeline_color);
        p.setColor(underlineColor);
        canvas.drawCircle(r.left+5,r.top +CURRENT_TIME_LINE_TOP_OFFSET-1,7,p);
        canvas.drawRect(r, p);
        ...
        //        }
        //huangyongwen@20161230 add for calendar new UI end
    }
    ...
    private void doDraw(Canvas canvas) {
        ...
        if (mFutureBgColor != 0) {
            drawBgColors(r, canvas, p); //绘制背景色
        }
        drawGridBackground(r, canvas, p);//绘制网格布局
        ...
        drawSelectedRect(r, canvas, p);// 绘制选择区域
    }
    ...
    private void drawSelectedRect(Rect r, Canvas canvas, Paint p) {
        ...
        if (mSelectionMode != SELECTION_HIDDEN && !mSelectionAllday) {
            ...
            p.setColor(mNewEventHintColor); // 白色
            if (mNumDays > 1) { //周视图时
                ...
                Log.d(TAG, "EVENT_TEXT_LEFT_MARGIN*4" + EVENT_TEXT_LEFT_MARGIN * 4 + "EVENT_TEXT_TOP_MARGIN" + EVENT_TEXT_TOP_MARGIN);
                float languageX = 0;
                float languageY = r.top + Math.abs(p.getFontMetrics().ascent) + EVENT_TEXT_TOP_MARGIN;
                float imageX = 0;
                float imageY = languageY - EVENT_TEXT_TOP_MARGIN*7 + ALLDAY_TOP_MARGIN;
                String language = Locale.getDefault().getLanguage();
                Log.d(TAG, "r.left" + r.left + "r.right" + r.right);
                if("ar".equals(language) || "fa".equals(language)){
                    p.setTextAlign(Align.RIGHT);
                    languageX = r.right - EVENT_TEXT_LEFT_MARGIN*6;
                    imageX = r.right - EVENT_TEXT_LEFT_MARGIN*5;
                    canvas.drawText(mNewEventHintString, languageX, languageY, p);
                }else {
                    languageX = r.left + EVENT_TEXT_LEFT_MARGIN*6;
                    imageX = r.left + EVENT_TEXT_LEFT_MARGIN;
                    canvas.drawText(mNewEventHintString, languageX, languageY , p);
                }
                Drawable drawable = getResources().getDrawable(R.drawable.new_activity);
                Bitmap bitmap = drawableToBitamp(drawable);
                canvas.drawBitmap(bitmap, imageX, imageY, p);
                //+ New event
            }
        }
    }
    ...
    private void drawGridBackground(Rect r, Canvas canvas, Paint p) {
        ...
        final float stopX = computeDayLeftPosition(mNumDays); // 计算距离左边的位置
        float y = 0;
        ...
        for (int hour = 0; hour <= 24; hour++) {
            mLines[linesIndex + 0] = GRID_LINE_LEFT_MARGIN;
            mLines[linesIndex + 1] = y;
            mLines[linesIndex + 2] = stopX;
            mLines[linesIndex + 3] = y;
            linesIndex += 4;
            y += deltaY;
        }

        Log.d(TAG, "drawGridBackground() arr1 [linesIndex] " + linesIndex);

        if (mCalendarGridLineInnerVerticalColor != mCalendarGridLineInnerHorizontalColor) {
            canvas.drawLines(mLines, 0, linesIndex, p); //绘制横线
            linesIndex = 0;
            Log.d(TAG, "drawGridBackground() arr3 [linesIndex] " + linesIndex);
            p.setColor(mCalendarGridLineInnerVerticalColor);
        }

        // Draw the inner vertical grid lines 绘制竖线
        for (int day = 0; day <= mNumDays; day++) {
            x = computeDayLeftPosition(day);
            mLines[linesIndex + 0] = x;
            mLines[linesIndex + 1] = startY;
            mLines[linesIndex + 2] = x;
            mLines[linesIndex + 3] = stopY;
            linesIndex += 4;
        }

        Log.d(TAG, "drawGridBackground() arr2 [linesIndex] " + linesIndex);

        canvas.drawLines(mLines, 0, linesIndex, p); //绘制竖线
    ...
    private void drawBgColors(Rect r, Canvas canvas, Paint p) {
        ...
        if (mNumDays == 1 && todayIndex == 0) {
            // Draw a white background for the time later than current time 当前时间之后的区域绘制成白色背景
...
//        }
    ...
    //手势监听，onTouchEvent首先捕获触摸事件，将这个事件，
    //传递给GestureDetector，然后交给GestureDetector分析。
    class CalendarGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            //比如在scroll滚动事件发生的时候，我们不断获取滚动点的水平偏移量，
            //然后响应的View也移动相同的偏移量，从而实现水平滑动
            return true;
        }
